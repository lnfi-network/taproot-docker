...
sudo docker exec -it -u bitcoin bitcoind-backend bitcoin-cli -regtest -rpcwallet=miningwallet -generate 120
...
sudo docker exec -it -u bitcoin bitcoind-backend bitcoin-cli -regtest -rpcwallet=miningwallet sendtoaddress $alice 100
...
bob=`sudo docker exec -it -u lnd bob-lnd lncli --network=regtest newaddress p2tr`
bob=$(echo $bob | awk -F'"' '{print $4}')
echo "bob address => $bob"

sudo docker exec -it -u bitcoin bitcoind-backend bitcoin-cli -regtest -rpcwallet=miningwallet sendtoaddress $bob 100
sudo docker exec -it -u bitcoin bitcoind-backend bitcoin-cli -regtest -rpcwallet=miningwallet -generate 6
